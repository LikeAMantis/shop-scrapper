...
async function insertCategorieProductSQL(shop: string, category: {name: string; id: string}) {
    var i = 0;
    ...
    await new Promise<void>((res) => {
        lineReader.eachLine(
            ...
            function (line, last) {
                ...
                const prodcutProps: ProductProps = {name: cells[1], price: cells[2], imgUrl: cells[3], productUrl: cells[4], categoryId: category.id};
...
function readCategories(shop: string): any[] {
    ...
    return categories.map(categorie => ({name: categorie, id: [...Math.random().toString(16).slice(10)].map(x => Math.random() > 0.5 ? x.toUpperCase() : x).join("")}));
}
